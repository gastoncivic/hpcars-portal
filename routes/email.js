...
async function sendEmail({ to, subject, html }) {
  ...
  try {
    const r = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({ from: `HP CARS <${FROM}>`, to, subject, html })
    });
    const d = await r.json();
    if (!r.ok) throw new Error(d.message || 'Error sending email');
...
function emailVerification(name, token) {
  ...
  return {
    ...
    html: `
<!DOCTYPE html><html><body style="margin:0;padding:0;background:#07080f;font-family:'Helvetica Neue',Arial,sans-serif">
<div style="max-width:560px;margin:40px auto;background:#0d0f1a;border:1px solid #1e2235;border-radius:16px;overflow:hidden">
  <div style="background:linear-gradient(135deg,#e8321a,#ff4d33);padding:32px;text-align:center">
    <h1 style="color:#fff;margin:0;font-size:28px;letter-spacing:2px">HP<span style="opacity:.8">CARS</span></h1>
    <p style="color:rgba(255,255,255,.8);margin:8px 0 0;font-size:13px">PORTAL PROFESIONAL</p>
  </div>
  <div style="padding:32px">
    <h2 style="color:#e8eaf0;margin:0 0 12px;font-size:20px">Hola, ${name} 👋</h2>
    ...
    <div style="text-align:center;margin:32px 0">
      <a href="${link}" style="background:#e8321a;color:#fff;padding:14px 36px;border-radius:8px;text-decoration:none;font-weight:700;font-size:15px;letter-spacing:1px;display:inline-block">✅ VERIFICAR MI CUENTA</a>
    </div>
    <p style="color:#7a8296;font-size:12px;line-height:1.6">Este link expira en 24 horas. Si no creaste esta cuenta podés ignorar este email.</p>
    <p style="color:#3a3f55;font-size:11px;margin-top:24px">O copiá este link en tu navegador:<br/><a href="${link}" style="color:#e8321a;word-break:break-all">${link}</a></p>
  </div>
  <div style="border-top:1px solid #1e2235;padding:16px 32px;text-align:center">
    <p style="color:#3a3f55;font-size:11px;margin:0">© 2024 HP CARS Portal · Chiptuning Profesional</p>
  </div>
</div>
</body></html>`
  };
...
function fileReceived(name, { service, brand, model, fileId }) {
  const serviceNames = { chiptuning: 'Chiptuning', immo: 'IMMO OFF', seedkey: 'Seed Key', special: 'Soluciones Especiales' };
  return {
...
<div style="max-width:560px;margin:40px auto;background:#0d0f1a;border:1px solid #1e2235;border-radius:16px;overflow:hidden">
  ...
  <div style="padding:32px">
    ...
    <div style="background:#12151f;border:1px solid #1e2235;border-radius:10px;padding:20px;margin:24px 0">
      <div style="display:flex;justify-content:space-between;margin-bottom:10px">
        <span style="color:#7a8296;font-size:13px">Servicio</span>
        <span style="color:#e8321a;font-weight:700;font-size:13px">${serviceNames[service] || service}</span>
      </div>
      <div style="display:flex;justify-content:space-between;margin-bottom:10px">
        <span style="color:#7a8296;font-size:13px">Vehículo</span>
        <span style="color:#e8eaf0;font-size:13px">${brand} ${model}</span>
      </div>
      <div style="display:flex;justify-content:space-between">
        <span style="color:#7a8296;font-size:13px">ID de archivo</span>
        <span style="color:#e8eaf0;font-size:13px">#${fileId}</span>
      </div>
    </div>
    <p style="color:#7a8296;font-size:13px">⏱️ Tiempo estimado de respuesta: <strong style="color:#f5a623">&lt; 4 horas hábiles</strong></p>
  ...
  <div style="border-top:1px solid #1e2235;padding:16px 32px;text-align:center">
    <p style="color:#3a3f55;font-size:11px;margin:0">© 2024 HP CARS Portal</p>
  </div>
</div>
</body></html>`
  };
...
<div style="max-width:560px;margin:40px auto;background:#0d0f1a;border:1px solid #1e2235;border-radius:16px;overflow:hidden">
  ...
  <div style="padding:32px">
    ...
    <div style="background:#12151f;border:1px solid rgba(46,213,115,.2);border-radius:10px;padding:20px;margin:24px 0">
      <div style="margin-bottom:8px"><span style="color:#7a8296;font-size:13px">Servicio: </span><span style="color:#2ed573;font-weight:700;font-size:13px">${serviceNames[service] || service}</span></div>
      <div style="margin-bottom:8px"><span style="color:#7a8296;font-size:13px">Vehículo: </span><span style="color:#e8eaf0;font-size:13px">${brand} ${model}</span></div>
